...
In 2008 Scott was awarded a U.S. Department of Energy Compuational Science Graduate Fellowship (CSGF) supporting his doctoral work at Cornell University Center for Applied Mathematics (CAM). He plans to move to San Francisco after graduation and has accepted a software engineering position at Yelp Inc.
\end{biosketch}
...
\end{center}
Every read $R_{k} \in \mathcal{R}$ is composed of two mate pairs $R_{k}^{(1)}$ and $R_{k}^{(2)}$ with corresponding lengths $L\left(R_{k}^{(1)}\right)$ and $L\left(R_{k}^{(2)}\right)$ drawn from a distribution $\Theta_{L_{k}}$. These mate pairs are separated by a distance $I_{k}$ drawn from a distribution $\Theta_{I_{k}}$.
...
\end{tabular}
\end{center}
Every read has a unique orientation based on how the enzymes in the sequencer bind to it. A read can be sequenced inward, towards the middle, which would imply $\omega(R_{k}) = +1$ in our notation. Alternately a read can be sequenced outward, from the middle to the ends, which would imply $\omega(R_{k}) = -1$.



Each read can map onto the seed (assembly) in the following ways,
\begin{center}
\textbf{Read $R_{k}$ \textit{Anchored} to Seed $S_{i}^{(t)}$} \\
\begin{tabular}{cc}
    %\text{Seed $S_{i}^{(t)}(51:88)$:} & $\cdots\overset{51}{\text{{\color{MyDarkGreen}C}}}$\text{{\color{orange} G}{\color{red} A}} $\overbrace{\text{{\color{red} A}}\overset{55}{\text{{\color{blue} T}}} \text{{\color{MyDarkGreen} C}{\color{orange} G}{\color{MyDarkGreen} C}{\color{MyDarkGreen} C}}\overset{60}{\text{{\color{blue} T}}}\text{{\color{orange} G}{\color{red} A}{\color{blue} T}{\color{blue} T}}\overset{65}{\text{{\color{MyDarkGreen} C}}}}^{D\left(R_{k}^{(1)},S_{i}^{(t)}\right) = \{54, \ldots, 65\}}\text{{\color{red} A}{\color{blue} T}{\color{blue} T}{\color{MyDarkGreen} C}}\overset{70}{\text{{\color{orange} G}}}$\\

    \text{Read $R_{k}$: } & \text{{\color{white}$\cdots$GCA}}$\underbrace{\underrightarrow{\text{{\color{red} A}{\color{blue} T}{\color{MyDarkGreen} C}{\color{orange} G}{\color{MyDarkGreen} C}{\color{MyDarkGreen} C}{\color{blue} T}{\color{orange} G}{\color{red} A}{\color{blue} T}{\color{blue} T}{\color{MyDarkGreen} C}}}}_{\Omega\left(R_{k}^{(1)},S_{i}^{(t)}\right) = +1}$\text{{\color{white} A}{\color{white} T}{\color{white} T}{\color{white} C}{\color{white} G}}
\end{tabular}

\[\left. I\left(R_{k}, S_{i}^{(t)}\right) > 5 \right| \omega_{k} = +1, \ \ \ \ \left. I\left(R_{k}, S_{i}^{(t)}\right) > 53 \right| \omega_{k} = -1\]

\textbf{Read $R_{k}$ \textit{Doubly Anchored} to Seed $S_{i}^{(t^{\prime})}$ at some later time $t^{\prime}$} \\
\begin{tabular}{cc} \\

%\text{Seed $S_{i}^{(t^{\prime})}(51:88)$:} & \text{$\cdots\overset{51}{\text{{\color{MyDarkGreen}C}}}${\color{orange} G}{\color{red} A}}$\overbrace{\text{{\color{red} A}$\overset{55}{\text{{\color{blue} T}}}${\color{MyDarkGreen} C}{\color{orange} G}{\color{MyDarkGreen} C}{\color{MyDarkGreen} C}$\overset{60}{\text{{\color{blue} T}}}${\color{orange} G}{\color{red} A}{\color{blue} T}{\color{blue} T}$\overset{65}{\text{{\color{MyDarkGreen} C}}}$}}^{D\left(R_{k}^{(1)},S_{i}^{(t^{\prime})}\right) = \{54, \ldots, 65\}}$\text{{\color{red} A}{\color{blue} T}{\color{blue} T}{\color{MyDarkGreen} C}$\overset{70}{\text{{\color{orange} G}}}${\color{red} A}{\color{orange} G}{\color{blue} T}{\color{MyDarkGreen} C}$\overset{75}{\text{{\color{orange} G}}}${\color{red} A}}$\overbrace{\text{{\color{blue} T}{\color{MyDarkGreen} C}{\color{red} A}$\overset{80}{\text{{\color{blue} T}}}${\color{MyDarkGreen} C}{\color{blue} T}{\color{MyDarkGreen} C}{\color{orange} G}$\overset{85}{\text{{\color{red} A}}}${\color{blue} T}{\color{orange} G}$\overset{88}{\text{{\color{MyDarkGreen} C}}}$}}^{\underset{\text{\scriptsize{$\left|D\left(R_{k}^{(2)},S_{i}^{(t^{\prime})}\right)\right| \geq \alpha_{K}$}}}{D\left(R_{k}^{(2)},S_{i}^{(t^{\prime})}\right) = \{77, \ldots, 88\}}}$\text{{\color{white} A}} \\

\text{Read $R_{k}$: } & \text{{\color{white}$\cdots$GCA}}$\underbrace{\underrightarrow{\text{{\color{red} A}{\color{blue} T}{\color{MyDarkGreen} C}{\color{orange} G}{\color{MyDarkGreen} C}{\color{MyDarkGreen} C}{\color{blue} T}{\color{orange} G}{\color{red} A}{\color{blue} T}{\color{blue} T}{\color{MyDarkGreen} C}}}}_{\Omega\left(R_{k}^{(1)},S_{i}^{(t^{\prime})}\right) = +1}$$\underbrace{\underset{\text{Implied Insert}}{\underleftrightarrow{\text{{\color{white} A}{\color{white} T}{\color{white} T}{\color{white} C}{\color{white} G}{\color{white} A}{\color{white} G}{\color{white} T}{\color{white} C}{\color{white} G}{\color{white} A}}}}}_{I\left(R_{k}, S_{i}^{(t^{\prime})}\right) = 12 }$$\underbrace{\underleftarrow{\text{{\color{blue} T}{\color{MyDarkGreen} C}{\color{red} A}{\color{blue} T}{\color{MyDarkGreen} C}{\color{blue} T}{\color{MyDarkGreen} C}{\color{orange} G}{\color{red} A}{\color{blue} T}{\color{orange} G}{\color{MyDarkGreen} C}{\color{red} A}}}}_{\Omega\left(R_{k}^{(2)},S_{i}^{(t^{\prime})}\right) = -1}$
\end{tabular}
\[O\left(R_{k}, S_{i}^{(t^{\prime})}\right) = \{1,2\}, \ \ \ \omega\left(R_{k}, S_{i}^{(t^{\prime})}\right) = +1\]
\end{center}
A read $R_{k}$ can be either \textit{anchored} or \textit{doubly anchored} to a seed $S_{i}^{(t)}$ if one or both of it's corresponding mate pairs match some subset of $S_{i}^{(t)}$ of size at least $\alpha_{k}$ exactly. If a read is singly anchored we can obtain the implied orientation with respect to the seed and bound the minimum insert size $I_{k}$ conditioned on the reads overall orientation $\omega_{k}$. If the read is doubly anchored we can directly calculate the implied total orientation $\omega_{k}$, insert length $I_{k}$ and ordering $O\left(R_{k}, S_{i}^{(t^{\prime})}\right)$.


\subsection{The ALE score and the likelihood of an assembly}
...
\subsection{Aprroximating $Z_{\text{placement}}$} % (fold)
...
% subsection Aprroximating $Z_{\text{placement}}$ (end)

\subsection{Aprroximating $Z_{\text{insert}}$} % (fold)
...
% subsection Aprroximating $Z_{\text{insert}}$ (end)

\subsection{Aprroximating $Z_{\text{depth}}$} % (fold)
...
\subsection{Aprroximating $Z_{\text{kmer}}$} % (fold)
...
Depth & 3909940 & 4481198\\\hline
\end{tabular}}{}
\end{table}

To determine whether these errors identified by ALE are true assembly errors or Illumina artifacts, we independently validated the results using PacBio sequencing data. A total of 53 SMRT cells comprising 221mb of mapped reads or 34 folds of overage were aligned to the assembly. Manual inspection of the resulting PacBio alignment confirms 5/5 assembly errors (Figure 5 B and C \& Table 1), suggesting the errors identified by ALE are true errors in the assembly. 
...
False Positive & - & - & 41318 & - & 13 & -\\
Del & TCATCGCG & - & 43200 & Did not find & 6 & 6\\
Ins & - & C & 47600 & 1 & 2 & 3 \\\hline
\end{tabular}}{}
\end{table*}
...
TODO put in proof writeup

\subsection{SAA multistart}
...
TODO
figure 2d grid, show points moving
...
GPGPU cores have very low memory per core/block (16KB for tesla, 4KB for x). Our algorithm uses matricies of various sizes,
...
The trivially MC portions of the algorithm only "need" the matricies of size $l \times l$ to compute their estimates, so that it all we send to the GPU, the calculations involving $n \times n$ are computed on the CPU where system memory is abundant.
...
requring memory of $O(lR)$ per run, $O(l)$ per GPU block.
...
[FIGURE 5,6,7 HERE]
 ...
 \centerline{\includegraphics[width=\textwidth]{figures/velvetrope/BAliBASEsnap2v2.png}}
...
See individual projects for licencing terms.

\subsection{Workstation}

All speed tests using wall clock time were performed on my personal workstation (originally built in 2008) with the following specifications
...
Some computations were performed on Hopper, a 153,216 processor (1.28PFlop) U.S. Department of Energy supercomputer at the National Energy Research Scientific Conmputing Center (NERSC), with the following specifications
...
\bibitem[Langmead {\it et~al}., 2009]{Langmead2009} Langmead,B., Trapnell,C., {\it et~al}. (2009) Ultrafast and memory-efficient alignment of short DNA sequences to the human genome, {\it Genome Biology}, {\bf 10}, R25.
